...
# Short runs to analyze the influence of the seeds.
...
def run_test(rng_mode: str, seed: int, mean: float, std: float, distribution_histogram: np.ndarray) -> dict[str, float | int]:
    ...
    statistics = tools.run(input_model, rng_mode, 1000)
...
output_file = "statistics/results1ExpSeed.txt"
...
# output_file = "statistics/results1LogSeed.txt"
...
# output_file = "statistics/results1GammaSeed.txt"
...
# output_file = "statistics/results1TriangularSeed.txt"
...
for random_mode in setup.prng_modes:
    # ThreadLocalRandom is not seedable
    if random_mode is 'ThreadLocalRandom':
        continue
    # Warm-up run
    print(random_mode, "warm-up")
    run_test(random_mode, 1000, mean, std, distribution_histogram)
    # Actual test runs
    for i in range(100):
        print(random_mode, i)
        result = run_test(random_mode, (i+1)*1000, mean, std, distribution_histogram)
